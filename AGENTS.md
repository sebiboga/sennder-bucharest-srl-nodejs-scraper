...
- **Only the API parsing logic in `index.js`** (`fetchGemJobs`) is sennder-specific. The output shape (`mapToJobModel`, `transformJobsForSOLR`) must stay uniform across derived scrapers.
