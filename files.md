...
| `src/job-validator.js` | Shared validation primitives - exports validateByHead(url), validateByContent(url, opts), DEFAULT_EXPIRED_KEYWORDS. Used by both `validate-jobs.js` and `tests/validate-sennder-jobs.js`. |
...
| `tests/company.json` | Mock ANAF company data for sennder used in unit tests |
| `tests/validate-sennder-jobs.js` | **sennder-specific fast validator (used by CI).** HEAD requests only, sennder CIF. Called nightly by `automation-testing.yml`. Supports `--dry-run` and `--delete`. |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real Gem ATS API, ANAF, and SOLR |
...
| `ROBOTS.md` | robots.txt analysis and scraping policy for sennder.com |
...
| `delete_request.json` | **Manual maintenance tool** — SOLR payload to delete ALL jobs for CIF 45780151. Use only when you need to wipe sennder jobs from SOLR entirely. Run with: `curl --user "${SOLR_AUTH}" "https://solr.peviitor.ro/solr/job/update?commit=true" -H "Content-Type: application/json" -d @delete_request.json` |
...
- Full workflow: validate company (ANAF+Peviitor) → scrape Gem ATS → transform → upsert SOLR → generate docs/jobs.md
